...
// namespace rewrite::benchmark {
    ...
    static void BM_Legacy_DustCalometryFile(benchmark::State& state) {
	...
	for (auto _: state)
	    comp.readCalorimetryFile(param, 0);
    }

    static void BM_Rewrite_DustCalometryFile(benchmark::State& state) {
	std::vector<double>	size_param(14);
	parameters 		param;
	CDustComponent	comp;
...
BENCHMARK(BM_Legacy_DustCalometryFile);
BENCHMARK(BM_Rewrite_DustCalometryFile);
